...
class EnhancedSafetyManager {
    ...
    async enforceProcessIsolation() {
        try {
            ...
            if (fsSync.existsSync(this.lockFile)) {
                ...
                try {
                    ...
                    try {
                        ...
                        // Lock file might have been deleted by another process - this is fine
                        if (unlinkError.code !== 'ENOENT') {
                            console.warn('⚠️ Warning: Could not remove lock file:', unlinkError.message);
                        } else {
                            console.log('🧹 Lock file already cleaned up');
                        }
                    }
                }
            }
            ...
            try {
                ...
                console.error('❌ Failed to create lock files:', writeError.message);
                throw new Error(`Cannot acquire process lock: ${writeError.message}`);
            }
            
        } catch (error) {
            console.error('❌ Process isolation check failed:', error.message);
            
            // If it's a lock file conflict, provide helpful guidance
            if (error.message.includes('Another bot instance is already running')) {
                console.error('💡 To resolve: Stop the other instance or wait for it to finish');
                console.error('💡 Check running processes: pm2 status or tasklist /fi "imagename eq node.exe"');
            }
    ...
    async canSendMessage(phone, orderId, messageType, messageContent, orderData = {}, sheetData = {}) {
        ...
        try {
            ...
            // 1.5. Developer Phone Check - Bypass all safety limits for developer phones
            console.log(`🔍 [${checkId}] Step 1.5: Developer Phone Check`);
            if (this.isDeveloperPhone(phone)) {
                await this.logger.logSafetyEvent('DEVELOPER_PHONE_CHECK', phone, orderId, messageType, 'BYPASSED', {
                    checkId,
                    reason: 'Developer phone - bypassing all safety limits'
                });
                console.log(`🔓 [${checkId}] Developer phone detected - bypassing all safety limits`);
                return this.createResult(true, 'DEVELOPER_PHONE_BYPASS', 'Developer phone - all safety limits bypassed', checkId);
            }
